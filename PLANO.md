...
- ✅ 31/08 — **Layout**: tema azul simples aplicado (`​.streamlit/config.toml` — antes era o azul padrão do Streamlit), logo do sistema configurável em **Configurações → Aparência** (upload de imagem, aparece no menu lateral em todas as telas via `st.logo`, guardada num bucket público novo do Supabase Storage chamado `assets` — usuário ainda precisa criar esse bucket), e as seções de leitura por IA em Contas a Pagar e Receber (boleto, comprovantes em lote) viraram expansores recolhidos por padrão — a tela não fica mais sobrecarregada logo de cara, "Novo lançamento" aparece sem precisar rolar.
- ✅ 31/08 — **Polimento visual "SaaS profissional"** (usuário achou o visual ainda amador, pediu referência tipo Omie): CSS global adicionado em `layout.py` (só com seletores estáveis do Streamlit — `data-testid`/`aria-current`, nunca as classes `st-emotion-cache-*`, que mudam de build em build e quebrariam o estilo silenciosamente). Métricas do Dashboard viraram cartões (fundo, borda, cantos arredondados) em vez de números soltos; expansores e tabelas ganharam borda/sombra sutil consistente; botões e campos com cantos levemente arredondados; menu lateral com item ativo destacado. No caminho, confirmado que o "vermelho" visto antes era só resquício de screenshot anterior ao tema existir — o tema azul já estava sendo aplicado certo desde o commit anterior.
- ✅ 31/08 — **Mockups aprovados levados pro Dashboard de verdade**: depois de validar o visual num protótipo (Claude Design — Dashboard, Importar OFX, Importar Comissão, Classificar Lançamentos, Contas a Pagar e Receber, Pesquisa Cliente), implementado no app.py real: `layout.cartoes_kpi()` (cartões com ícone + selo, substituindo o st.metric nativo) e `graficos.py` (gráfico de fluxo de caixa em linha e rosca de comissões por status, em SVG desenhado à mão, sempre calculados a partir de dados reais do Supabase — nunca inventados). Bug real encontrado e corrigido: o Markdown do Streamlit interpreta linhas com 4+ espaços de indentação como bloco de código — o HTML dos cartões/legenda (escrito como f-string indentada) aparecia cru na tela a partir do 2º elemento. Corrigido com `_compacto()` (remove indentação de cada linha antes do `st.markdown`). Validado rodando de verdade contra o Supabase real — cartões, gráfico e rosca todos com os números reais atuais. 6 novos testes (115 no total).
- **Semana 6 — status:** segurança ✅, backup ✅, anexos + SharePoint ✅, leitura de boleto por IA ✅ (validada contra arquivo real), baixa automática por comprovante ✅ (testada pelo usuário com arquivos reais, com desempate de ambiguidade), revisão de ponta a ponta ✅, datas/valores em formato brasileiro ✅, estrutura de integração bancária preparada (Pluggy, pendente conta real do usuário pra validar), layout + polimento visual ✅ (Dashboard já com o visual novo rodando; outras telas — Importar OFX, Importar Comissão, Classificar Lançamentos, Contas a Pagar e Receber, Pesquisa Cliente — com mockup aprovado, ainda pendentes de implementar). Restam: mais testes com arquivos reais (depende do usuário enviar mais demonstrativos) e preparação final para uso.
