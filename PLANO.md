...
- **Semana 6 — status:** segurança ✅, backup ✅, anexos + SharePoint ✅, leitura de boleto por IA ✅ (validada contra arquivo real), baixa automática por comprovante ✅ (testada pelo usuário com arquivos reais, com desempate de ambiguidade), revisão de ponta a ponta ✅, datas/valores em formato brasileiro ✅, estrutura de integração bancária preparada (Pluggy, pendente conta real do usuário pra validar), layout (tema + logo + telas menos carregadas) ✅. Restam: mais testes com arquivos reais (depende do usuário enviar mais demonstrativos) e preparação final para uso.
